...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, (samples, class_id) in tqdm(enumerate(data_loader)):
        ...
        if args.cuda is not None:
            ...
            mae_loss, pred, mask, disc_loss, adv_loss, currupt_img,batch_loss = model(samples.to(device), class_id.to(device),mask_ratio=args.mask_ratio)
        else:
            mae_loss, pred, mask, disc_loss, adv_loss, currupt_img,batch_loss = model(samples,class_id, mask_ratio=args.mask_ratio)
        ...
        loss_scaler(batch_loss, optimizer, parameters=model.parameters(),
                    update_grad=(data_iter_step + 1) % accum_iter == 0, retain_graph = True)
        ...
        metric_logger.update(batch_loss=batch_loss_value)
        ...
        batch_loss_value_reduce = misc.all_reduce_mean(batch_loss_value)
        ...
        if log_writer is not None and (data_iter_step + 1) % accum_iter == 0:
            ...
            log_writer.add_scalar('batch_loss', batch_loss_value_reduce, epoch_1000x)
